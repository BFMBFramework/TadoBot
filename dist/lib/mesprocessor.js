...
Your homes are: ${self.getHomeNameList().join(", ")}`;
        ...
        if (commandArgs.length > 1) {
            ...
            if (homeZoneArray.length > 1) {
                let homeId = self.getHomeIdBy(homeZoneArray[0]);
                let zoneId = self.getZoneIdBy(homeId, homeZoneArray[1]);
                clientClass.getJaysonClient().request('receiveMessage', { token: clientClass.getToken(), network: 'Tado', options: { api_method: 'getZoneOverlay', home_id: homeId, zone_id: zoneId } }, function (err, response) {
                    ...
                    clientClass.getJaysonClient().request('sendMessage', { token: clientClass.getToken(), network: 'Telegram', options: options }, function (err, response) {
                        if (err) {
                            ...
                            logger_1.logger.info("Zone overlay result sent.");
                        }
                    });
                });
                return;
...
Your homes are: ${self.getHomeNameList().join(", ")}`;
        ...
        if (commandArgs.length > 1) {
            ...
            if (homeZoneArray.length > 1) {
                ...
                clientClass.getJaysonClient().request('receiveMessage', {
                    ...
                    options: { api_method: 'setZoneOverlay', home_id: homeId, zone_id: zoneId, power: "off", temperature: "5", termination: "auto" }
    ...
    clearZoneOverlayCommand(message) {
        const clientClass = client_1.Client.sharedInstance;
        const self = client_1.Client.sharedInstance.getMessageProcessor();
...
Your homes are: ${self.getHomeNameList().join(", ")}`;
        ...
        if (commandArgs.length > 1) {
            ...
            if (homeZoneArray.length > 1) {
                let homeId = self.getHomeIdBy(homeZoneArray[0]);
                let zoneId = self.getZoneIdBy(homeId, homeZoneArray[1]);
                clientClass.getJaysonClient().request('receiveMessage', { token: clientClass.getToken(), network: 'Tado', options: { api_method: 'clearZoneOverlay', home_id: homeId, zone_id: zoneId } }, function (err, response) {
                    ...
                    clientClass.getJaysonClient().request('sendMessage', { token: clientClass.getToken(), network: 'Telegram', options: options }, function (err, response) {
                        if (err) {
                            ...
                            logger_1.logger.info("Clear overlay result sent.");
                        }
                    });
                });
                return;
